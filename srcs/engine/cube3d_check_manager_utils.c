...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2020/04/15 15:43:42 by gozsertt          #+#    #+#             */
/*   Updated: 2020/04/20 18:09:47 by gozsertt         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
											...
											t_map map, t_sprites **lst_sprite)
{
	...
	result = ((map.size.y * 0.4f) * 10.0f) / result;
	pixel_h = (map.size.y / 2.0f) - (result / 2.0f);
	pixel_b = (map.size.y / 2.0f) + (result / 2.0f);
	nbr[0] = 0;
	while (nbr[0] < pixel_h)
		put_pixel(g_app->image, 
            create_vector2(info.column, nbr[0]++), map.board[0][0]->ceiling);
	nbr[0] = pixel_b;
	while (nbr[0] < map.size.y)
		put_pixel(g_app->image, 
